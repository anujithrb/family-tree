...
beforeAll(async () => {
  const tree = await prisma.familyTree.create({ data: { name: `relationship-test-${Date.now()}` } });
  treeId = tree.id;
});
...
async function makeFamily() {
  const grandpa = await prisma.person.create({ data: { name: 'Grandpa', birth: 1920, gender: 'M', treeId } });
  const grandma = await prisma.person.create({ data: { name: 'Grandma', birth: 1922, gender: 'F', treeId } });
  const couple1 = await prisma.couple.create({ data: { spouseAId: grandpa.id, spouseBId: grandma.id } });
  const dad = await prisma.person.create({ data: { name: 'Dad', birth: 1950, gender: 'M', treeId } });
  await prisma.coupleChild.create({ data: { coupleId: couple1.id, childId: dad.id, sortOrder: 0 } });
  const mom = await prisma.person.create({ data: { name: 'Mom', birth: 1952, gender: 'F', treeId } });
  const couple2 = await prisma.couple.create({ data: { spouseAId: dad.id, spouseBId: mom.id } });
  const child = await prisma.person.create({ data: { name: 'Child', birth: 1975, gender: 'M', treeId } });
...
async function makeCousins() {
  ...
  const dad = await prisma.person.create({ data: { name: 'Dad', birth: 1950, gender: 'M', treeId } });
  await prisma.coupleChild.create({ data: { coupleId: couple1.id, childId: dad.id, sortOrder: 0 } });
  const uncle = await prisma.person.create({ data: { name: 'Uncle', birth: 1952, gender: 'M', treeId } });
  await prisma.coupleChild.create({ data: { coupleId: couple1.id, childId: uncle.id, sortOrder: 1 } });
  const mom = await prisma.person.create({ data: { name: 'Mom', birth: 1953, gender: 'F', treeId } });
  const couple2 = await prisma.couple.create({ data: { spouseAId: dad.id, spouseBId: mom.id } });
  const aunt = await prisma.person.create({ data: { name: 'Aunt', birth: 1955, gender: 'F', treeId } });
  const couple3 = await prisma.couple.create({ data: { spouseAId: uncle.id, spouseBId: aunt.id } });
  const cousin1 = await prisma.person.create({ data: { name: 'Cousin1', birth: 1978, gender: 'M', treeId } });
  await prisma.coupleChild.create({ data: { coupleId: couple2.id, childId: cousin1.id, sortOrder: 0 } });
  const cousin2 = await prisma.person.create({ data: { name: 'Cousin2', birth: 1980, gender: 'F', treeId } });
...
describe('GET /api/relationship', () => {
  ...
  test('returns 404 when no path exists between disconnected people', async () => {
    const alice = await prisma.person.create({ data: { name: 'Alice', birth: 1970, gender: 'F', treeId } });
    const bob   = await prisma.person.create({ data: { name: 'Bob',   birth: 1972, gender: 'M', treeId } });
